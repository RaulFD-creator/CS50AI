...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        # Parental information provided - use conditional probability
            ...
            num_genes_mother = count_genes(people[person]['mother'], one_gene, two_genes)
            num_genes_father = count_genes(people[person]['father'], one_gene, two_genes)

            # Only 1 way to inherit 0 copies: inherit 0 copies from each parent
            if num_genes == 0:
                probability *= prob_inherite(num_genes_mother, False) * prob_inherite(num_genes_father, False)
            
            # Two ways to inherit 1 copy: 1 from mother and 0 from father, or vice versa
            elif num_genes == 1:
                probability *= prob_inherite(num_genes_mother, True) * prob_inherite(num_genes_father, False) \
                                + prob_inherite(num_genes_mother, False) * prob_inherite(num_genes_father, True)
            
            # Only 1 way to inherit 2 copies: inherit 1 copy from each parent
            elif num_genes == 2:
                probability *= prob_inherite(num_genes_mother, True) * prob_inherite(num_genes_father, True)
...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    for person in probabilities:
        # Count number of copies of the gene of interest that person has
        num_gene = count_genes(person, one_gene, two_genes)
...
def prob_inherite(num_gene_parent, inherited):
    """
    Subfunction within joint_probability to calculate the probability
    that a given descendent has inherited a given number of copies
    from their parents.
    """
    if num_gene_parent == 0:
        if inherited:
            return PROBS["mutation"]
        else:
            return (1-PROBS["mutation"])
    elif num_gene_parent == 1:
        return 0.5

    elif num_gene_parent == 2:
        if inherited:
            return 1 - PROBS["mutation"]
        else:
            return PROBS["mutation"]
