...
def compute_idfs(documents):
    ...
    for word in words:
        ...
        idf = np.log(num_documents / count_word) 
        idfs[word] = idf
...
def top_files(query, files, idfs, n):
    ...
    for file, words in data:
        tf_idf = 0
        for word in query:
            tf_idf += words.count(word) * idfs[word]
        score[file] = tf_idf

    ranked_files = sorted(score.items(), key=lambda x: x[1], reverse = True)
...
def top_sentences(query, sentences, idfs, n):
    ...
    for sentence, words in data:
        ...
        count = sum(map(lambda x: x in words_in_query, words))
        density_query = count / len(words)
        scores[sentence] = {
            'IDF': idf, 
            'Query_Term_Density': density_query
        }
    
    ranked_sentences = sorted(scores.items(), key = lambda x: (x[1]['IDF'], 
        x[1]['Query_Term_Density']), reverse = True)
