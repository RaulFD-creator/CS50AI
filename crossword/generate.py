...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        """
        # Loop through each variable and initialise an empty set of words that violates
        # node consistency
        ...
        # Loop through each word in variable's domain and check for node consistency
            ...
            # If the word has violated node consistency, remove it from the variable's domain
    ...
    def revise(self, x, y):
        ...
        if overlap is not None:
            ...
            for word_in_x in self.domains[x]:
                overlap_x = word_in_x[overlap[0]]
                image_y = {word_in_y[overlap[1]] for word_in_y in self.domains[y]}
                
                # If there's no possible value in y's domain that doesn't cause conflict,
                # need to remove that word from x's domain
                if overlap_x not in image_y:
                    removable_words.add(word_in_x)
                    revised = True
        ...
        while queue:
            ...
            if self.revise(x, y):
                ...
                if not self.domains[x]:
                    print("Unsolvable Problem. Ending AC-3")
                    return False
    ...
    def consistent(self, assignment):
        ...
        # Inconsistent if not all values are distinct, values are not distinct if there are more or
        # less values than keys
    ...
    def order_domain_values(self, var, assignment):
        ...
        # Set neighbors of var
        neighbors_var = self.crossword.neighbors(var)

        # Consider every word in var's domain
        for word_var in self.domains[var]:

            # Loop through each of var's unassigned neighbors
            for neighbor in (neighbors_var - assignment.keys()):
                overlap = self.crossword.overlaps[var, neighbor]

                # Loop through each word in neighbor's domain
                for word_new in self.domains[neighbor]:

                    # Count number of choices eliminated if there is conflict between word in var's
                    # domain and word in neighbor's dommain
                    if word_var[overlap[0]] != word_new[overlap[1]]:
                        num_choices_eliminated[word_var] += 1
        
        # Sort var's domain in ascending order of the number of values they rule out
    ...
    def select_unassigned_variable(self, assignment):
        ...
        return values.
        """
        # Unassigned variables would be those variables within self.crossword once the keys of assignment
        # are removed
        ...
        # If there is no tie, return variable with minimum number of remaining values
        ...
        if sorted_num_remaining_values[0][1] == sorted_num_remaining_values[1][1] and len(sorted_num_remaining_values) > 1:
    ...
    def backtrack(self, assignment):
        ...
        """
        # Check assignment to verify whether it is complete
